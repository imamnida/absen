...
class Wizin extends CI_Controller {
    ...
    private function absen_process($action, $id_kelas) {
        ...
        $is_registered_uid = $this->W_Izin_Model->is_registered_uid($uid);
        ...
        $is_already_absent = $this->W_Izin_Model->is_already_absent($uid, $action);

        if ($is_already_absent) {
            $data['message'] = 'Anda sudah melakukan absensi '.$action.' sebelumnya hari ini.';
        } else {
            if ($action == 'masuk') {
                $this->W_Izin_Model->absen_masuk($uid, $id_devices);
                $data['message'] = 'Absensi masuk berhasil.';
            } elseif ($action == 'keluar') {
                $this->W_Izin_Model->absen_keluar($uid, $id_devices);
                $data['message'] = 'Absensi keluar berhasil.';
            } elseif ($action == 'izin') {
                $this->W_Izin_Model->absen_izin($uid, $id_devices);
                $data['message'] = 'Absensi izin berhasil.';
            } elseif ($action == 'sakit') {
                $this->W_Izin_Model->absen_sakit($uid, $id_devices);
